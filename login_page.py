"""
login_page.py - Production Ready Login & Signup with Stripe Integration
"""

import streamlit as st
from auth import sign_in, sign_up, create_stripe_checkout, upgrade_tier
...
def show_login_page():
    ...
    with col:
        st.markdown('<div class="hero-logo">TRACK<span style="color:#ff3b3b;">FORM</span></div>', unsafe_allow_html=True)
        st.markdown('<div class="hero-subtitle">AI TECHNIQUE COACH</div>', unsafe_allow_html=True)
        ...
        with tab2:
            ...
            if st.button(button_label, key="btn_signup", use_container_width=True):
                if not new_email or not new_password:
                    ...
                    if result.get("success"):
                        if plan == "free":
                            st.success("✅ Account created successfully! Please check your email to confirm, then sign in.")
                        else:
                            # Auto sign in to get user object for Stripe
                            ...
                            if sign_result.get("success"):
                                ...
                                if checkout.get("success") and checkout.get("url"):
                                    ...
                                    <div style="background:#111;border:2px solid #22c55e;border-radius:12px;padding:2rem;text-align:center;margin:1.5rem 0;">
                                        <h3 style="color:#22c55e;">Account Created Successfully!</h3>
                                        <p>Click the button below to complete your {plan.upper()} subscription.</p>
                                    ...
                                    st.error("Account created but payment link failed. Please contact support.")
                            else:
                                st.success("Account created! Please sign in.")
                    else:
                        st.error(result.get("error", "Signup failed. Please try again."))